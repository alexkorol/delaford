...
export default {
  ...
  'wagon:outfit:buy': async ({ data }, ws) => {
    ...
    let delivered = null;
    try {
      // overflow: 'reject' — a full backpack must fail the delivery outright
      // (triggering the refund below) instead of dropping the purchase at the
      // player's feet while the ledger is also credited back.
      delivered = await player.inventory.add(itemId, 1, { overflow: 'reject' });
    } catch (error) {
